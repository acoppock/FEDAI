...
Helper functions that are about estimation rather than about the book now live
in [estimatrTools](https://github.com/acoppock/estimatrTools). None of them are
re-exported from this package, so book code that uses them should load that
package directly.
...
`lm_bootstrap()` and `tidy.lm_bootstrap()` used to live in `R/bootstrap.R`.
...
(rsample's "Bootstrap001") is now an integer `replicate`. Two things changed
underneath. Resampling is base R rather than rsample, dplyr, purrr, tidyr and
rlang, which is why this package no longer imports any of them. And arguments
passed through `...` are forwarded to `lm_robust` unevaluated instead of being
evaluated against the **original** data first: a `weights = w` used to be paired
with the resampled rows in the order the original data held it, rather than the
weight belonging to the row that was drawn.
